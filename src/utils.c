...
char *stringify_shape(size_t *shape, size_t ndim, size_t buffer_size) {
    ...
    for (size_t dim = 1; dim < ndim - 1; dim++) {
        bytes += snprintf(shape_buff + bytes, buffer_size - bytes, " %zu,", shape[dim]);
    }

    if (ndim > 1) {
        bytes += snprintf(shape_buff + bytes, buffer_size - bytes, " %zu", shape[ndim - 1]);
    }
...
void *safe_malloc(size_t size, char *file, int line) {
    ...
    if (ptr == NULL) {
        fprintf(stderr, 
        "Could not allocate %zu bytes (File: %s, line: %d)\n", 
        size, file, line);
        exit(1);
    }
